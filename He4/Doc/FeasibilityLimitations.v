...
the form [exec_step_relation -> term -> state -> Prop], but we must provide a
concrete [exec_step_relation], [term], and [state] to get a [Prop]. And when we
prove that [Prop], we prove it only for that particular combination of
[exec_step_relation], [term], and [state].
